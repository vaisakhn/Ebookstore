...
const Main=()=>{
    ...
    const searchBook=(evt)=>{
        if(evt.key==="Enter")
        {
            axios.get('https://www.googleapis.com/books/v1/volumes?q='+search+'&key=AIzaSyAAcOkgyBhDgpseVB13iWzT913OUuc-30M')
    ...
    <div style={{display:'flex',flexDirection:'row',maxWidth:'28rem',minWidth:'fit-content'}} class="container mx-auto ">
        ...
        <button onClick={()=>{ axios.get('https://www.googleapis.com/books/v1/volumes?q='+search+'&key=AIzaSyAAcOkgyBhDgpseVB13iWzT913OUuc-30M')
            .then(res=>setData(res.data.items))
...
<div class='flex'><button onClick={()=> {
            axios.get('https://www.googleapis.com/books/v1/volumes?q='+search+'=free-ebooks&key=AIzaSyAAcOkgyBhDgpseVB13iWzT913OUuc-30M')
            .then(res=>setData(res.data.items))
            .catch(err=>console.log(err))
...
<button onClick={()=> {
            axios.get('https://www.googleapis.com/books/v1/volumes?q='+search+'=paid-ebooks&key=AIzaSyAAcOkgyBhDgpseVB13iWzT913OUuc-30M')
            .then(res=>setData(res.data.items))
            .catch(err=>console.log(err))
